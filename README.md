...
- **Concurrency Safety** — `select_for_update()` + DB-level `UniqueConstraint` prevents double-booking
...
| Doctor 2 | dr.mehta@yopmail.com | doctor123 |
...
```
doctor_booking/
├── doctor_booking/         # Django project config
│   ├── settings.py
│   ├── urls.py
...
│   ├── superadmin_views.py # Template-based dashboard views
│   ├── urls.py
│   └── superadmin_urls.py
│
...
1. **Application level** — `select_for_update()` acquires a row-level DB lock before checking slot availability
2. **Database level** — `UniqueConstraint` on `(doctor, appointment_date, start_time)` filtered by active statuses

### Slot Generation
Slots are generated dynamically in `doctors/utils.py:generate_slots_for_doctor()`. The algorithm:
...
4. Iterates from `start_time` to `end_time` in `slot_duration` increments
